...
//	private TollMetricRegistry tollMetricRegistry;
	...
	public ResponseEntity getTollCustomerDetails(final User user, final String bankReferenceId, final String status,
			Long startTime, Long endTime, Integer start, Integer count, String vehicleNumber, String accountNumber ,String serialNumber ,String branchCode,String phone)
			throws Exception, APIException {
		...
		if (bankId != null && status != null && startTime == null) {
			...
			if (!CollectionUtils.isEmpty(listTollTagByStatus)) {
				...
				for (TollTag tag : listTollTagByStatus) {
					...
					Bank issuerbank = bankDBService.getBank(tag.getWalletBankId() > NumberUtils.INTEGER_ZERO 
							? tag.getWalletBankId() :  tag.getBankId());
	...
	public ResponseEntity getTollTagCharges(final User user, final String encTtid) throws Exception, APIException {
		...
		if (user != null && encTtid != null) {
			...
			List<FeesAndDeposit> feesAndDeposit = feesAndDepositDBService.findByBankIdAndChargeCategory(
					Arrays.asList(tollTag.getWalletBankId() != NumberUtils.INTEGER_ZERO ? tollTag.getWalletBankId() :tollTag.getBankId()), new HashSet<>(Arrays.asList(tollTag.getCategory())));
	 ...
	 */
    public ResponseEntity updateTollCustomerByVendor(final User user,
			final Bank bank, final TollTagRequest updateReq, final String clientTransactionId)
			throws Exception, APIException {
		...
		if (new HashSet<>(Arrays.asList(ArrayUtils.isNotEmpty(environment.getActiveProfiles())
                ? environment.getActiveProfiles()
                : environment.getDefaultProfiles())).contains("prod")
				&& (StringUtils.isEmpty(epc.getTid()) || StringUtils.isEmpty(epc.getSignData()))) {
		...
		if (StringUtils.isBlank(tollTag.getTagId()) && StringUtils.isBlank(tollTag.getTid())) {
			...
			if (new HashSet<>(Arrays.asList(ArrayUtils.isNotEmpty(environment.getActiveProfiles())
              ...
              if(new HashSet<>(Arrays.asList(ArrayUtils.isNotEmpty(environment.getActiveProfiles())
                  ? environment.getActiveProfiles()
                      : environment.getDefaultProfiles())).contains("uat")){
                tollTagCloneCopy.setTid(epc.getTid());
                tagNPCIApprovalStatus.setTid(epc.getTid());
              }else {
			...
			response.addHeaderCode("TC-999", "Enter this -> "+tollTag.getSerialNumber() + " Serial Number as this "
					+ "vehicle has already been assigned this -> "+tollTag.getSerialNumber() + "Serial Number on previous approval");
		...
		if (tollProperties.isConnectNpci() && isTagIdBlank) {
		  LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
              .message("BEFORE CALLING MANAGE TAG").format());  
			
			if(StringUtils.isEmpty(tollTagCloneCopy.getEngineNo()) && StringUtils.isEmpty(tollTagCloneCopy.getVinNumber())) {
				//call reqVehicleDetails api first get the vahan related details and then hit manageTag api with those details 
				LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
						.message("hitting  MANAGE TAG 1.0").format());	

				String last5DigitsOfEngineNo = null;
				if(StringUtils.isNotEmpty(tollTagCloneCopy.getEngineNo()) && tollTagCloneCopy.getEngineNo().length() > 5) {
					 last5DigitsOfEngineNo = tollTagCloneCopy.getEngineNo().substring(tollTagCloneCopy.getEngineNo().length() - 5);
				}
				// manage tag api version 1.0 commenting as we will be using manage tag version 1.1 for vahan integration		
				/*
				...
				for (TollTagUpdateResponse.Txn.Resp.Tag tag : tollTagUpdateResponse.getTxn().getResp().getTag()) {
					...
					// manage tag api version 1.0 commenting as we will be using manage tag version 1.1 for vahan integration		
						...
						tollTagUpdateResponse = uploadTheTollListToNETCManageTag(httpServletContext, Arrays.asList(tollTagCloneCopy),
								TollConstant.UPDATE_OP, bank, merchants.get(0));
						...
						tollTagUpdateResp = objectMapper.readValue(objectMapper.writeValueAsString(tollTagUpdateResponse),
								TollTagUpdateResponse.class);
						for (TollTagUpdateResponse.Txn.Resp.Tag tags : tollTagUpdateResponse.getTxn().getResp().getTag()) {
							tag = tags;
			...
			if (tollTag.getRegistrationAmount().compareTo(BigDecimal.ZERO) > NumberUtils.INTEGER_ZERO
				...
				List<Dipcoin> dcoins = dipcoinDBService
						.asyncFindDipcoin(tollTag.getCustomerAccountId(), Arrays.asList(
								DBConstants.DipcoinUsageType.DEPOSIT.value(), DBConstants.DipcoinUsageType.FEE.value()))
						.get();
				if (CollectionUtils.isEmpty(dcoins)) {
					...
					for (Dipcoin dcoin : dcoins) {
						if (StringUtils.isNotBlank(dcoin.getUsageCategory())
							...
							dcoinReq.setOrderId(dcoin.getUsageCategory() + DateTime.now(DateTimeZone.UTC).getMillis());
							...
							try {
								processDcoinResponse = partnerDipcoinResource.processCustomerDipcoin(user, bank, dcoinReq,
										TransactionSource.WEB, Boolean.FALSE);
							} catch (Exception e) {
								LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
			...
			if (bankAPIServices.getBankProperties(bank.getReferenceId()).isTollTagApprovalSet()) {
				...
				if (tollRegistration != null && !createdByUser.getRole().equals(DBConstants.UserRoles.CUSTOMER.value())) {
					tollTagCloneCopy.setStatus(String.valueOf(TollTagApprovalStatus.ACTIVE.value()));
				} else {
					tollTagCloneCopy.setStatus(String.valueOf(TollTagApprovalStatus.CUSTOMER_ACTIVATION_PENDING.value()));
				}
			}
			if (!ostaProcessed) {
			...
			if (tollTagResponse == null) {
				...
				removeLock(userProfileLock, serialNumberLock);
				LOG.debug(LogFormatter.instance(httpServletContext.getTraceId()).message("updatetollTag is null").format());
				response.addHeaderCode(HeaderCode.INTERNAL_ERROR);
				return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
			}
			if (isTagIdBlank) {
			...
			return ResponseEntity.status(HttpStatus.OK).body(response);
			  ...
			  LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
                  .message("hitting  MANAGE TAG 1.1").format());  
				...
				if(StringUtils.isNotEmpty(tollTagCloneCopy.getEngineNo()) && tollTagCloneCopy.getEngineNo().length() > 5) {
					 last5DigitsOfEngineNo = tollTagCloneCopy.getEngineNo().substring(tollTagCloneCopy.getEngineNo().length() - 5);
			            ...
			            TollTagUpdateResponse tollTagUpdateResponse = asyncUploadTheTollListToNETCManageTag(httpServletContext,
				                Arrays.asList(tollTagCloneCopy), TollConstant.ADD_OP, bank, merchants.get(0),tagNPCIApprovalStatus);
					 if (tollTagUpdateResponse == null) {
							tollTagDeepCopy.setTollRegistration(tollRegistration);
							if (this.tollDBService.updateTollTag(tollTagDeepCopy) == null) {
								LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	        ...
	        //This is done because these data should not get set before NPCI response other wise tag will get wasted.
...
//	        }
	    ...
	    String tollTagUpdateRespJson = "{"
	    	    + "\"head\": {"
	    	    + "    \"ver\": \"1.1\","
	    	    + "    \"ts\": \"2024-03-21T16:16:12\","
	    	    + "    \"orgId\": \"brio\","
	    	    + "    \"msgId\": \"brio21MAR24\""
	    	    + "},"
	    	    + "\"txn\": {"
	    	    + "    \"id\": \"BAOIWUHFCYR0PGTEQWANEP\","
	    	    + "    \"note\": \"ADD\","
	    	    + "    \"refId\": \"\","
	    	    + "    \"refUrl\": \"https://192.168.6.190/\","
	    	    + "    \"ts\": \"2024-03-21T16:16:12\","
	    	    + "    \"type\": \"ManageTag\","
	    	    + "    \"resp\": {"
	    	    + "        \"ref\": null,"
	    	    + "        \"merchantId\": null,"
	    	    + "        \"ts\": \"2024-03-21T16:16:17\","
	    	    + "        \"result\": \"SUCCESS\","
	    	    + "        \"respCode\": \"000\","
	    	    + "        \"tag\": ["
	    	    + "            {"
	    	    + "                \"op\": \"ADD\","
	    	    + "                \"tagId\": \"34161FA8202F874608000318\","
	    	    + "                \"seqNum\": \"1\","
	    	    + "                \"result\": \"SUCCESS\","
	    	    + "                \"errCode\": \"240\""
	    	    + "            }"
    ...
    public TollTagUpdateResponse uploadTheTollListToNETCManageTag(final HttpServletContext httpServletContext,
			...
			Merchant merchant /* ,ReqVehicleDetailResponse reqVehicleDetailResponse */)
		...
		for (TollTag tollTagRequest : tollTagRequests) {
			...
			if (operation.equalsIgnoreCase(TollConstant.ADD_OP)) {
				...
				// ******** Commented By Murad, If RegNum is not empty in NETC Request then set T (told by Sanskruti)
		//		detailList.add(new Detail(TollConstant.REGISTERED_VEHICLE, tollTagRequest.getRegisteredVehicle())); 
				// ******** 
				
				//detailList.add(new Detail(TollConstant.REGISTERED_VEHICLE, "T"/*tollTagRequest.getRegisteredVehicle()*/));
			...
			//	detailList.add(new Detail(TollConstant.STATE, tollTagRequest.getState()));
				//detailList.add(new Detail(TollConstant.STATE, "MH"/*tollTagRequest.getState()*/));
				
				//should be NEW_TAG|REPLACEMENT_TAG
			//	detailList.add(new Detail(TollConstant.TAGDESCRIPTOR, tollTagRequest.getTagDescriptor()));
				//detailList.add(new Detail(TollConstant.TAGDESCRIPTOR, "REPLACEMENT_TAG"/*tollTagRequest.getTagDescriptor()*/));
...
*/
			...
			// for managetag 1.1 registrationNo is optional and vin is mandatory that is why added this check

			// commented by Murad -> if we don't have reg no then pass starting 20 digit of VIN (Sanskruti Mane)
...
//			}
	      ...
	      //  detailList.add(new Detail(TollConstant.VIN, tollTagRequest.getVinNumber()));
	        //detailList.add(new Detail(TollConstant.VIN, "5KL8M8ED6B0108371"/*tollTagRequest.getVinNumber()*/));
			
			//detailList.add(new Detail(TollConstant.ENGINENO, tollTagRequest.getEngineNo()));
			//detailList.add(new Detail(TollConstant.ENGINENO, "JH4G589HGB54842158"/*tollTagRequest.getEngineNo()*/));
		...
		//	detailList.add(new Detail(TollConstant.VEHICLEDESCRIPTOR, tollTagRequest.getVehicleDescriptor()));
			//detailList.add(new Detail(TollConstant.VEHICLEDESCRIPTOR, reqVehicleDetailResponse.getVEHICLEDESCRIPTOR()));
			
			//T|F
	//		detailList.add(new Detail(TollConstant.NATIONALPERMIT, tollTagRequest.getNationalPermit()));
			//detailList.add(new Detail(TollConstant.NATIONALPERMIT, "T"/*tollTagRequest.getNationalPermit()*/));
...
//			}
			...
			//detailList.add(new Detail(TollConstant.PERMITEXPIRYDATE, "15-12-2028"/*String.valueOf(tollTagRequest.getPermitExpiryDate())*/));
...
public ResponseEntity fetchApprovalstatus(String serialNumber) throws Exception  {
    	...
    	if(serialNumber==null) {
    		LOG.error(LogFormatter.instance(httpServletContext.getTraceId())
                    .message("SerialNumber is empty").format());
 			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(APIResponse.error(HeaderCode.BAD_REQUEST));
    	...
    	RespVehicleDetails  respMngTagResp = objectMapper.readValue(tolltag.getAddTagResponse(),RespVehicleDetails.class);
    	...
    	if(respMngTagResp.getTxn().getResp().getTag().get(0).getResult().equalsIgnoreCase("SUCCESS")) {
    		...
    		LOG.error(LogFormatter.instance(httpServletContext.getTraceId())
                  .message("Tag Result is Success").format());
		 return ResponseEntity.status(HttpStatus.OK).body(responseCharges);
    	}else {
            try {
                List<HeaderCode> headerCodes = new ArrayList<>();
               ...
               // LOG.error("Error occurred while processing error codes: " + e.getMessage()).;
                LOG.error(LogFormatter.instance(httpServletContext.getTraceId()).message("Error occurred while processing error codes: ")
						.data("Error",errCodes).format());
...
public ResponseEntity vehicleVerification(User user, String vehicleRegistrationNo, String tagId, String tid,
	...
	if (!tollProperties.isConnectNpci()) {
		...
		if (CollectionUtils.isNotEmpty(tollTags)) {
         ...
         for (TollTag tollTag : tollTags) {
           if (!(TollTagApprovalStatus.INACTIVE.value() == Integer
               .parseInt(tollTag.getStatus())
               || TollTagApprovalStatus.BANK_REJECTED.value() == Integer
                   .parseInt(tollTag.getStatus()))
             ...
             LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
                 ...
                 .data("TollTag Status:", tollTag.getStatus())
	...
	//return brontooResource.getVehicleInfo(user, vehicleRegistrationNo, tagId, tid, engineNo, vinNumber, vrn, last5digitofengineno, regType, bankReferenceId);
}

public ResponseEntity reqVehicleDetails(String vrn, String vin, String last5digitofenginNumber, Integer bankId, String bankReferenceId) {
	   ...
	   LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	  	          .message("vrn" +vrn).data("last5digitofengineno", last5digitofenginNumber).data("vin", vin).format());
	   ...
	   if(StringUtils.isNotEmpty(last5digitofenginNumber) && last5digitofenginNumber.length() > 5){
	    ...
	    try {
	      ...
	      LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
		          .data("vin is not blank ",vin).format());
	     
	     responseData = this.callNETCForReqVehicleDetails(vrn, vin, last5digitofengineno, bank, refUrl, tollNetcSyncTimeResponse);          	     
...
/* 
 * reVehicleDetails 1.1 This API is called by Issuer Bank to get the details of registered Tag ID using VIN & 
       last5digitofengineno OR using VRN & last5digitofengineno.
           Privilege: Initiated by Issuer Bank
 */
public String callNETCForReqVehicleDetails(String vrn, String vinNumber, String last5digitofengineno, Bank bank, String refUrl,
	      TollNetcSyncTimeResponse tollNetcSyncTimeResponse) throws Exception {

	TollNetcSyncTimeResponse tollNetcSyncTimeResponse1 = new TollNetcSyncTimeResponse();
	
	LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	          .message("vrn" +vrn).data("last5digitofengineno", last5digitofengineno).data("vinNumber", vinNumber).format());
	...
	if(refUrl == null) {
	    ...
	    if(StringUtils.isNotBlank(vrn) && StringUtils.isNotBlank(last5digitofengineno)) {
	    	LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	    	          .message("vrn" +vrn).data("last5digitofengineno", last5digitofengineno).format());
	    	vehicle.setVrn(vrn);
	    	...
	    	LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	    	          .message("vinNumber" +vinNumber).data("last5digitofengineno", last5digitofengineno).format());
...
//get Dipcoin transaction
			...
			final String status, Long startTime, Long endTime, Integer start, Integer count)throws Exception, APIException 
	{
		...
		if (bankId != null && phone != null) {
			...
			listOfTollTag = this.tollDBService.getByBankIdAndTollRegistrationAndStatus(bankId, tollRegistrationIds, status);
...
//		listOfTollTag = this.tollDBService.getByBankIdAndTollRegistrationDobAndStatus(bankId, branchCode, status);
		...
		if (registration != null) {
			for (TollTag tolltag : tollTag) {
				...
				Dipcoin dipcoin = dipcoinDBService.findDipcoin(tolltag.getCustomerAccountId(),
						DBConstants.DipcoinStatus.ACTIVE.value(), DBConstants.DipcoinUsageType.TOLL.value());
				Bank bank = bankDBService.getBank(tolltag.getWalletBankId() > NumberUtils.INTEGER_ZERO 
							? tolltag.getWalletBankId() :  tolltag.getBankId());
				List<User> makerUser = userDBService
	...
	public static void populateTollTagDetailsResponse(final User user, final TollTag tollTag, TollTagResponse response,
		...
		if (tollTag != null) {
			...
			response.setWalletFlag(tollTag.getWalletBankId() > NumberUtils.INTEGER_ZERO ? NumberUtils.INTEGER_ONE : NumberUtils.INTEGER_ZERO );
	   ...
	   public TollTagUpdateResponse asyncUploadTheTollListToNETCManageTag(final HttpServletContext httpServletContext,
	            List<TollTag> tollTagRequests, String operation, Bank bank,
	            Merchant merchant, TagNPCIApprovalStatus tagNPCIApprovalStatus)
	            throws APIException, Exception {
...
//	                  : tollTagRequest.getVinNumber());
	        ...
	        if (tagNPCIApprovalStatus != null) {
	          ...
	          if (tagNPCIApprovalStatusDBService
	              .addTagNPCIApprovalStatus(tagNPCIApprovalStatus) == null) {
	            return null;
	          }
...
//	              return tollTagUpdateResponse;
	        ...
	        try {
	            ...
	            tollHttpsServices.send(url, httpServletContext.getTraceId(),
	                    byteArrayOutputStream);
