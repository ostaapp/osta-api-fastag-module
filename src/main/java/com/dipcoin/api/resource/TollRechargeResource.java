...
public class TollRechargeResource {
	...
	private static final Logger LOG = LogManager.getLogger(TollRechargeResource.class);
	  ...
	  /**
	   * addRechargeAmount is adding the recharge into the recharge table. It will find the active row
	   * based on customerAccountId and Status=1 and INACTIVE the row And add the entry and mark it
	   * active
	   */
...
//	      }
	      if (tollTag == null) {
	    	  LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	                  .message("TollTag is null Pratik").format());
...
//	      }
	  ...
	  private static TollRecharge populateForSaveAndUpdateTollCustomer(
		      HttpServletContext httpServletContext, User user, String clientTransactionId,
		      TollRechargeRequest rechargeReq, TollTag tollTag, User bankUser) {
	   ...
	   * 
	   */
	  public ResponseEntity tollRechargeCreateOsta(final User user,
	      final TollRechargeRequest rechargeReq, String clientTransactionId, User bankUser)
	    ...
	    CustomerAccount customerAccount =
	        customerDBService.asyncGetAccount(user.getId(), rechargeReq.getCardId()).get();
	    ...
	    if ((StringUtils.isEmpty(rechargeReq.getAuthorizationPin()) && !rechargeReq.getAuthorizationPin().equalsIgnoreCase(APIConstants.PIN))
	    ...
	    if(customerAccount.getTypeOfMethod() == CustomerAccountMethodType.WALLET_INB.value() || customerAccount.getTypeOfMethod() == CustomerAccountMethodType.BANK_SDK.value()) {
	    	rechargeReq.setAuthorizationPin(APIConstants.PIN);
	    }
...
//	 	   	}    	
	// }
	    ...
	    if (CollectionUtils.isEmpty(merchants)) {
	      LOG.debug(LogFormatter.instance(httpServletContext.getTraceId()).message("merchant is null")
	          .format());
	      return ResponseEntity.status(HttpStatus.BAD_REQUEST)
	          .body(APIResponse.error(HeaderCode.BAD_REQUEST));
	    }
	    ...
	    if (dipcoin != null) {
	      ...
	      try {
	    	  httpServletContext.setClientTransactionId(clientTransactionId.concat(StringUtils.SPACE )
	    	        .concat(rechargeReq.getInitiatedFrom()).concat(StringUtils.SPACE) + (bankUser !=null ? bankUser.getId() : user.getId()));
	      ...
	      deleteDipcoinResponse =
	          customerDipcoinResource.deleteDipcoin(user, user.getPhone().concat(dipcoin.getCoin()),false);     
	      
	      } catch (APIException e) {
	      
	      LOG.debug(
	          LogFormatter.instance(httpServletContext.getTraceId())
	              .message("Active Toll Dcoin Which will be deleted Exception").format(),
	          e);
	      boolean holdmarked = false;
	     if(e.getResponse()!= null) {
	    	 
	    	 for(ResponseHeader responseHeader : e.getResponse().getCodes()) {
	       	  if( responseHeader.getCode().toUpperCase().contains(HeaderCode.BANK_HOLD_MARK_FOR_HOLD_DOES_NOT_EXIT.code().toUpperCase())
	           		  || responseHeader.getMessage().toUpperCase().contains(HeaderCode.BANK_HOLD_MARK_FOR_HOLD_DOES_NOT_EXIT.message().toUpperCase())) {
	       		holdmarked = true;
	     ...
	     if(holdmarked) {
	      ...
	      if (deleteDipcoinResponse.getStatusCodeValue() >= HttpStatus.BAD_REQUEST.value()) {
	        LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	            .message("Dipcoin Not Deleted").format());
	    ...
	    if(customerAccount.getTypeOfMethod() == CustomerAccountMethodType.BANK_SDK.value() && customerAccount.getBank().getIsSsoEnabled() == BooleanStatus.YES.value()) {
	    ...
	    LOG.debug(LogFormatter.instance(httpServletContext.getTraceId())
	        .message("create dipcoin response")
	    ...
	    if (createDipcoinresponse.getStatusCodeValue() >= HttpStatus.BAD_REQUEST.value()) {
	      ...
	      ResponseEntity updateExcCodeResponse = updateExceptionCode(TollConstant.ADD_OP,customerAccount,dipcoin,user,null,rechargeReq,coinResponse);
	      if(lock != null)
	      lock.unlock();
	    ...
	    if (coinResponse == null) {
	     ...
	     return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(customerDipcoinResponse);
	      ...
	      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(customerDipcoinResponse);
		    ...
		    ResponseEntity updateExcCodeResponse = updateExceptionCode(TollConstant.REMOVE_OP,customerAccount,dipcoin,user,null,rechargeReq,coinResponse);
		    ...
		    if (responseBody instanceof TollTag) {
		        ...
		        LOG.error("Received APIResponse instead of TollTag - ",
	                    apiResponse);
	    ...
	    try {
	       addRechargeResponse =
	          this.addRechargeAmount(user, clientTransactionId, tollrequest, customerAccount, bankUser);
	    ...
	    if(tollTag != null && tollTag.getAvailableAmount().compareTo(tollTag.getMinimumAmount()) < NumberUtils.INTEGER_ZERO) {
...
//				}
		  ...
		  public ResponseEntity revertDipcoin(User user, TollRechargeRequest rechargeReq,
			    ...
			    if (coinResponse != null) {
			      ...
			      ResponseEntity deleteDipcoinResponse =
			          customerDipcoinResource.deleteDipcoin(user, coinResponse.getOsta(), false);
			    ...
			    ResponseEntity createDipcoinResponse =
			        customerDipcoinResource.createDipcoin(user, createReq, false);
